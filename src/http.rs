...
fn walk_html_dom(handle: Handle, found: bool, c: &mut usize) -> Option<String> {
    let node = handle;
    ...
    *c += 1;
    ...
    for child in node.children.borrow().iter() {
        if let Some(title) = walk_html_dom(child.clone(), title_found, c) {
            return Some(title);
        }
    }
...
/// Parse title using a DOM generated by html5ever
fn parse_title_html(page_contents: &str) -> Option<String> {
    ...
    let ret = walk_html_dom(dom.document, false, &mut recursion_count);
    trace!("parsed HTML with {} DOM recursions", recursion_count);
...
fn parse_title(page_contents: &str) -> Option<String> {
    let title_dec = match parse_title_html(page_contents) {
        Some(t) => t,
        None => return None,
    };
